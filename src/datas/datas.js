const datas = {
  home: {
    title: "Oh ! Hey there sunshine ",
    description:
      "I’m Ugo Olsak, a<span class='bold'> fourth year student at HETIC</span> and <span class='bold'>freelancer</span>.<br> Even though the digital environment is for me like water for a fish, I grew up wondering why things were made the way they are (...consider me a very self aware fish).<br> This quest led me to an emerging passion, <span class='bold'>design.</span>",
    caseStudy: {
      title: "Case Studies",
      description:
        "A quick selection of my work as an intern, freelance and student.",
      cases: [
        {
          year: "2018",
          role: "UX/UI",
          context: "Freelance",
          name: "Tesla X TripAdvisor",
          slug: "tesla-x-tripadvisor",
          emojis: [
            "../img/projects/tesla/emoji.png",
            "../img/projects/tesla/emoji@2x.png",
            "../img/projects/tesla/emoji@0,5x.png"
          ],
          imgs: [
            "../img/projects/tesla/tesla-img.jpg",
            "../img/projects/tesla/tesla-img.jpg",
            "../img/projects/tesla/tesla-img.jpg"
          ],
          forme: "../img/home/form_1.svg"
        },
        {
          year: "2018",
          role: "UX/UI",
          context: "Freelance",
          name: "La Débraillée",
          slug: "la-debraillee",
          imgs: [
            "../img/projects/la-debraillee/la-debraillee.png",
            "../img/projects/la-debraillee/la-debraillee@2x.png",
            "../img/projects/la-debraillee/la-debraillee@3x.png"
          ],
          forme: "../img/home/form_2.svg"
        },
        {
          year: "2018",
          role: "UX/UI",
          context: "Freelance",
          name: "Economy",
          slug: "economy",
          imgs: [
            "../img/projects/economy/economy.png",
            "../img/projects/economy/economy@2x.png",
            "../img/projects/economy/economy@3x.png"
          ],
          forme: "../img/home/form_3.svg"
        }
      ],
      subSection: {
        surtitle: "By the way",
        title: "I’m looking for a 6 month internship starting in june 2019.",
        contact: "Contact me",
        instruction: "Click to copy to clipboard"
      }
    },
    freelance: {
      surtitle: "Available for freelance",
      title: "To the moon and back",
      contact: "contact me",
      instruction: "Click to copy to clipboard",
      description:
        "Always on the look for new opportunities.<br> Once again feel free to "
    },
    face: {
      title: "Putting a face on a folio",
      description:
        "Just so you know what I look like,<br> in case we ever met.",
      img: ""
    },
    experimentsBloc: {
      title: "Last but not least, some experiments",
      description:
        "Since i’m still a student I like learning (always will) and trying stuff.<br>  Here is a quick selection of some of my experiments so far.",
      experiments: [
        {
          imgs: [
            "../img/experiments/panjin.png",
            "../img/experiments/panjin@0,5x.png"
          ],
          date: "08.11.2018",
          credit: "PAN/JIN dribbble shot",
          link: "https://dribbble.com/shots/5583031-PAN-JIN-shot-preview"
        },
        {
          imgs: [
            "../img/experiments/urbanjungle.png",
            "../img/experiments/urbanjungle@0,5x.png"
          ],
          date: "18.02.2018",
          credit: "Hannah in the house - Urban jungle bloggers",
          link:
            "https://dribbble.com/shots/4238966-Hannah-in-the-house-Urban-jungle-bloggers"
        },
        {
          imgs: [
            "../img/experiments/moniamerlot.png",
            "../img/experiments/moniamerlot@0,5x.png"
          ],
          date: "25.01.2018",
          credit: "Monia Merlo - Philosophical beauty",
          link:
            "https://dribbble.com/shots/4150613-Monia-Merlo-Philosophical-beauty"
        }
      ]
    },
    thanks: {
      title: "Thanks folk",
      text: "Can't wait to join your team : ",
      contact: "olsak.ugo@gmail.com",
      instruction: "Click to copy to clipboard"
    }
  },
  commun: {
    contact: "olsak.ugo@gmail.com",
    social: {
      twitter: "https://twitter.com/ugoolsak",
      behance: "https://www.behance.net/ugoolsak",
      dribbble: "https://dribbble.com/UgoOlsak",
      linkedin: "https://www.linkedin.com/in/ugoolsak/"
    }
  }
};

export { datas };
